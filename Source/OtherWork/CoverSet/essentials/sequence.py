...
class Sequence:
    ...
    def sanitize(sequence):
        """Remove consecutive equal optimizations. Example:
    ...
    def index_pass_to_list(sequence_index,
                           passes):
        """Convert a index sequence into a pass sequence. Example:
    ...
    def index_pass_to_string(sequence_index,
                             passes):
        """Convert a index sequence into a string. Example:
    ...
    def name_pass_to_string(sequence):
        """Convert a list of names to string. Example:
    ...
    def get_the_best(sequences,
        ...
        if len(sequences) < nof_sequences:
            lg.error('There is no {} sequences '.format(
                nof_sequences)
            )
            sys.exit(1)
